...
func NewConfigEnv() *ConfigEnv {
	...
	return NewConfigEnvBase(hostName, cmdName, "")
}
...
func NewConfig(home string, configEnv *ConfigEnv) *Config {
	...
	sslDir := filepath.Join(varDir, "network")

	pidFile := fmt.Sprintf("_pid_%s", configEnv.programName)
	pid := os.Getpid()
	workDir := fmt.Sprintf("_%d", pid)
	configFile := configEnv.configFile
	if configFile == "" {
		configFile = filepath.Join(home, "gcagent.toml")
	}
	// プロセス定義
	...
	config.ParameterFile = configFile          // パラメータファイル
	config.ProgramName = configEnv.programName // プログラム名

	// ディレクトリ定義
	config.OutDir = filepath.Join(varDir, "data")          // 採取データディレクトリ
	config.WorkDir = filepath.Join(varDir, "tmp", workDir) // ワークディレクトリ
	config.WorkCommonDir = filepath.Join(varDir, "tmp")    // 共有ワークディレクトリ
...
func (config *Config) GetConfigPath() string {
	return filepath.Join(config.Home, config.ParameterFile)
}
