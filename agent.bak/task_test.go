...
func TestWindowsTask(t *testing.T) {
	...
	task := NewTask(job)
	c := NewConfig("../testdata/ptune/", NewConfigEnv())
	_, err := toml.DecodeFile(c.ParameterFile, c.Schedule)
	if err != nil {
		t.Error(err)
	}
	job, ok := c.Schedule.Jobs["windowsconf"]
	if !ok {
		t.Error("not found windowsconf job")
	}
	datastore, err := c.NewDatastore("windowsconf", time.Now(), TIME_OFF)
	if err != nil {
		t.Error(err)
	}
