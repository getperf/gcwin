...
func (config *Config) SaveReport(task *Task, ds *Datastore) error {
	...
	log.Debug("make report ", ds.RelDir())
...
func (config *Config) ArchiveData(task *Task, ds *Datastore) error {
	...
	return Zip(zipPath, config.OutDir, ds.RelDir())
}
...
func (config *Config) RunTask(c *Collector, count int, tasks chan<- *Task) {
	...
	defer cancel()
	ch := make(chan bool, 1)
	ds, err := config.NewDatastoreCurrent(c.StatName)
	if err != nil {
		log.Error("new out log ", err)
	}
	task := NewTask(c, ds.AbsDir(), config.ScriptDir)
	task.Datastore = ds
	id := ds.RelDir()
