...
func GetHostname() (string, error) {
	...
	if err != nil {
		return hostName, errors.Wrap(err, "get hostname")
	}
...
func GetParentAbsPath(inPath string, parentLevel int) (string, error) {
	...
	if err != nil {
		return inPath, errors.Wrap(err, fmt.Sprintf("get parent absolute path %s", inPath))
	}
...
func CopyFile(srcPath, targetPath string) error {
	...
	defer src.Close()
	if err != nil {
		return errors.Wrap(err, fmt.Sprintf("copy from source %s", srcPath))
	}

	dst, err := os.Create(targetPath)
	defer dst.Close()
	if err != nil {
		return errors.Wrap(err, fmt.Sprintf("copy to target %s", targetPath))
	}
